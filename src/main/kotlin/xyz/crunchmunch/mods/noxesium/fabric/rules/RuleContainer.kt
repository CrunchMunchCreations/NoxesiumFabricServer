...
data class RuleFunction<T : Any>(
    val index: Int,
    val constructor: (Int) -> RemoteServerRule<T>,
)

/** Stores rule objects for an object. */
data class RuleHolder(
    /** All rules sent to this client. */
    val rules: MutableMap<Int, RemoteServerRule<*>> = ConcurrentHashMap(),
...
data class RuleContainer(
    ...
    val contents: Map<Int, RuleFunction<*>>
        get() = rules

    /** Returns whether rule [index] is available on [version]. */
    fun isAvailable(index: Int, version: Int,): Boolean =
    ...
    fun register(index: Int, minimumProtocol: Int, ruleSupplier: RuleFunction<*>,) {
    ...
    fun <T : Any> create(index: Int, storage: MutableMap<Int, RemoteServerRule<*>>, version: Int? = null,): RemoteServerRule<T>? {
